...
namespace Football {
  ...
  AudioThread::AudioThread() : wxThread::wxThread(wxTHREAD_JOINABLE), sound(nullptr), work(true), cond(mutex) {}
    ...
    public:
	  ...
	  void visit(GameObject &obj) override {
	    ...
	    dc.DrawBitmap(bitmap,
				static_cast<wxCoord>(pos.x * this->size.x - sz.x / 2), static_cast<wxCoord>(pos.y * this->size.y - sz.y / 2));
	  }
	private:
	  wxDC &dc;
	  wxSize size;
  ...
  FootballCanvas::FootballCanvas(wxWindow *parent, wxWindowID id, int8_t fps, std::string &background)
    : wxWindow::wxWindow(parent, id), background(), projector(5000.0f), fpsSum(0.0f), fpsCount(0) {
  ...
  FootballStatus FootballCanvas::getStatus() {
	return this->status;
  }
  
  void FootballCanvas::enableSounds(bool snd) {
	  this->playSound = snd;
  }
  
  void FootballCanvas::enableProjectors(bool pr) {
	this->projectors = pr;
  }
