...
AttackerController::AttackerController(ControllableObject &obj,
...
void AttackerController::play(PassiveObject &ball, float millis) {
  ...
  if (this->distanceTo(ball.getPosition()) > HIT_DISTANCE) {
    ...
    if (rand() % 101 < accuracy) {
      this->smartKick(ball, this->team.getKickDestination(),
                      this->kickStrength);
    } else {
      this->smartKick(ball, this->team.getRandomPlayer().getPosition(),
                      this->kickStrength);
    }
  }
}

AttackerControllerFactory::AttackerControllerFactory(TeamCollection &game,
                                                     Team &team, float speed,
                                                     float kickStrength)
    : PlayerControllerFactory(game, team, speed, kickStrength) {}

std::unique_ptr<ObjectController>
AttackerControllerFactory::create(ControllableObject &obj, KickHandler *kick) {
  std::unique_ptr<AttackerController> ctrl =
      std::make_unique<AttackerController>(obj, this->gameObjects, this->team,
