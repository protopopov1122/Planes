...
namespace Football {
	...
	DefenderController::DefenderController(ControllableObject &obj, TeamCollection &game, Team &team, float speed, float kickStrength)
	...
	DefenderControllerFactory::DefenderControllerFactory(TeamCollection &game, Team &team, float speed, float kickStrength)
		: PlayerControllerFactory(game, team, speed, kickStrength) {}
	
	std::unique_ptr<ObjectController> DefenderControllerFactory::create(ControllableObject &obj, KickHandler *kick) {
		std::unique_ptr<DefenderController> ctrl = std::make_unique<DefenderController>(obj, this->gameObjects, this->team, this->speed, this->kickStrength);
