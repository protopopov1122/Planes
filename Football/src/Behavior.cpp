...
namespace Football {
	...
	BehaviorController::BehaviorController(ControllableObject &obj, GameObjectCollection &gameObjects, float speed)
		: ObjectController::ObjectController(obj), gameObjects(gameObjects), speed(speed), randomPosition(0.0f, 0.0f), smartChasePosition(0.0f, 0.0f) {}
	...
	bool BehaviorController::chase(float millis, Position &destination, float energy, bool avoidCollisions) {
		...
		if (hyp > BARRIER) {
			...
			for (float i = 0; i < speed && hypot(destination.x - position.x, destination.y - position.y) > BARRIER; i += STEP) {
	...
	void BehaviorController::smartChase(float millis, Position &destination, float energy) {
		...
		if ((fabs(destination.x + this->smartChasePosition.x - curPos.x) <= RANGE &&
			...
			this->smartChasePosition = Position(radius * cos(angle), radius * sin(angle));
		}
	...
	void BehaviorController::random(float millis, float from, float to, float energy) {
		if (from > to) {
			float temp = to;
			to = from;
			from = temp;
		...
		if (!this->chase(millis, this->randomPosition, energy)) {
			this->randomPosition = Position(from + (to - from) / (rand() % 5 + 1), 0.01f * (rand() % 100 + 1));
		}
	}
