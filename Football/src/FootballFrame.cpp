...
namespace Football {
  ...
  FootballFrame::FootballFrame(std::string background)
    : wxFrame::wxFrame(nullptr, wxID_DEFAULT, "Football", wxDefaultPosition, wxSize(800, 600)) {
	  ...
	  wxCheckBox *soundCheckbox = new wxCheckBox(controlPanel, wxID_ANY, "Enable sound");
	  ...
	  wxCheckBox *projectorsCheckbox = new wxCheckBox(controlPanel, wxID_ANY, "Enable projectors");
	  ...
	  this->speedSlider = new wxSlider(controlPanel, wxID_ANY, 0, -5, 5, wxDefaultPosition, wxDefaultSize, wxSL_HORIZONTAL | wxSL_LABELS | wxSL_AUTOTICKS);
	  ...
	  this->goals = new wxTextCtrl(controlPanel, wxID_ANY, "", wxDefaultPosition, wxDefaultSize, wxTE_READONLY | wxTE_MULTILINE);
	  controlSizer->Add(this->goals, 1, wxEXPAND);
	  
	  this->canvas->Bind(FOOTBALL_STATUS_CHANGE, &FootballFrame::OnStatusChange, this);
	  this->canvas->Bind(FOOTBALL_SCORE_CHANGE, &FootballFrame::OnScoreChange, this);
  ...
  FootballCanvas *FootballFrame::getCanvas() {
    return this->canvas;
  }
  
  void FootballFrame::updateScore() {
	std::pair<uint16_t, uint16_t> score = wxGetApp().getGame().getScore();
	this->score->SetLabel(std::to_string(score.first) + "x" + std::to_string(score.second));
  ...
  void FootballFrame::ChangeStatus(wxCommandEvent &evt) {
	this->canvas->setStatus(this->canvas->getStatus() == FootballStatus::Paused ?
		FootballStatus::Playing : FootballStatus::Paused);
  }
  ...
  void FootballFrame::OnStatusChange(wxCommandEvent &evt) {
	if (this->canvas->getStatus() == FootballStatus::Projector ||
		...
		this->pauseButton->SetLabel(this->canvas->getStatus() == FootballStatus::Paused ? "Run" : "Pause");
  ...
  void FootballFrame::OnScoreChange(wxCommandEvent &evt) {
	...
	for (auto &goal : wxGetApp().getGame().getGoals()) {
		*this->goals << wxString("Team " + std::to_string(goal.first) + " Player " + std::to_string(goal.second) + "\n");
	}
  }
  ...
  void FootballFrame::updateTiming() {
	...
	this->fpsText->SetLabel("FPS: " + std::to_string(static_cast<int>(this->canvas->getFPS())));
  }
  ...
  void FootballFrame::registerKick() {
	 ...
	 if (kick.first != nullptr && kick.second != nullptr) {
		this->SetStatusText("Team " + std::to_string(kick.first->getID()) + " Player " + std::to_string(kick.second->getID())); 
	 }
  }
}