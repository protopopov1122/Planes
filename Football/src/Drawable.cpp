...
Drawable::Drawable(wxBitmap &base, wxSize &visualSize)
    : size(visualSize), rotation(0.0f) {
...
Drawable::Drawable(const Drawable &drawable)
    : size(drawable.size), rotation(0.0f) {
...
wxSize &Drawable::getSize() {
  ...
  wxSize currentSize = wxSize(this->size.GetWidth() * canvasSize.GetWidth() /
                                  Drawable::baseScreenSize.GetX(),
                              this->size.GetHeight() * canvasSize.GetHeight() /
                                  Drawable::baseScreenSize.GetY());
...
float Drawable::getRotation() const { return this->rotation; }
...
wxBitmap &Drawable::getBitmap() { return this->current; }
...
void Drawable::setBaseScreenSize(wxSize sz) { Drawable::baseScreenSize = sz; }
}