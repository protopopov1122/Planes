...
namespace Football {
	...
	Game::Game() : kickHandler(nullptr), speed(1.0f), ball(nullptr), lastKick(std::make_pair(nullptr, nullptr)) {}

	void Game::setKickHandler(KickHandler &h) {
		this->kickHandler = &h;
	}
	...
	void Game::visit(GameVisitor &v, Position &center, float radius, bool includeBall) {
		if (this->ball.get() != nullptr && includeBall) {
	...
	void Game::setBall(std::unique_ptr<Ball> ball) {
		this->ball = std::move(ball);
	}
	
	bool Game::collision(Position &pos, ControllableObject &obj, wxSize &sz) {
		if ((this->teams.first != nullptr && this->teams.first->playerCollides(obj, pos, sz)) ||
			(this->teams.second != nullptr && this->teams.second->playerCollides(obj, pos, sz))) {
			return true;
		}
		for (std::size_t i = 0; i < this->referees.size(); i++) {
	...
	std::vector<std::pair<size_t, size_t>> &Game::getGoals() {
		return this->goals;
	}
	...
	bool Game::checkGoals() {
		...
		if (this->checkGoal(this->teams.first->getGoal(), ballPosition)) {
			...
			if (this->lastKick.first != nullptr &&
				this->lastKick.second != nullptr) {
				this->goals.push_back(std::make_pair(this->lastKick.first->getID(), this->lastKick.second->getID()));
			}
			return true;
	...
	bool Game::checkGoal(Rect &goal, Position &pos) {
		return pos.x >= goal.x - goal.w / 2 &&
			pos.y >= goal.y &&
			pos.x <= goal.x + goal.w * 1.5f &&
			pos.y <= goal.y + goal.h;
	}
	
	std::pair<uint16_t, uint16_t> Game::getScore() {
		return std::make_pair(this->teams.first->getParameters().getScore(), this->teams.second->getParameters().getScore());
	}
	...
	std::pair<Team *, Player *> Game::getLastKick() {
