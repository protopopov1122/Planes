...
namespace Football {
    ...
    bool FootballApp::OnInit() {
		...
		BallControllerFactory balls(0.1f, Rect(0.035f * 1.25, 0.05f, 1.0f - 2.5f * 0.035f, 1.0f - 2 * 0.05f));
		this->game.setBall(std::make_unique<Ball>(ballDraw, balls));
		this->game.getBall()->setPosition(Position(0.5f, 0.25f));
		std::unique_ptr<Team> team1 = std::make_unique<Team>(this->game, this->game, std::move(team1Draw), Position(1.0f, 0.5f), Rect(0.04f, 0.45f, 0.008f, 0.1f), 1);
		team1->getLayout().addActivePlayer(ActivePlayerType::Attacker, Position(0.7f, 0.5f));
		team1->getLayout().addActivePlayer(ActivePlayerType::Attacker, Position(0.8f, 0.9f));
		team1->getLayout().addActivePlayer(ActivePlayerType::Defender, Position(0.1f, 0.5f));
		team1->getLayout().addActivePlayer(ActivePlayerType::Halfdefender, Position(0.3f, 0.5f));
		
		std::unique_ptr<Team> team2 = std::make_unique<Team>(this->game, this->game, std::move(team2Draw), Position(0.0f, 0.5f), Rect(1 - 0.04f - 0.008f, 0.45f, 0.008f, 0.1f), 2);
		...
		LinearRefereeControllerFactory linReferees(this->game, 0.1f, linRefereeOffset);
		std::unique_ptr<Referee> linRef1 = std::make_unique<Referee>(refereeDraw, linReferees);
		linRef1->getPosition().y = linRefereeOffset;
		game.addReferee(std::move(linRef1));
		linReferees.setLine(1 - linRefereeOffset);
		std::unique_ptr<Referee> linRef2 = std::make_unique<Referee>(refereeDraw, linReferees);
		...
		this->frame->getCanvas()->loadSounds(this->resourceManager->getFullPath("goal.wav"),
	...
	Game &FootballApp::getGame() {
		return this->game;
	}
	
	FootballFrame *FootballApp::getFrame() {
		return this->frame;
	}
}
