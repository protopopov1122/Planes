...
namespace Football {
	...
	GameObject::GameObject(const Drawable &drawable) : drawable(drawable), position(0.5f, 0.5f) {}
	...
	ObjectController *GameObject::getController() {
	...
	float GameObject::getRotation() const {
		return this->drawable.getRotation();
	}
	
	void GameObject::setRotation(float rot) {
		this->drawable.setRotation(rot);
	}
	...
	bool GameObject::collision(ControllableObject &obj, ControllableObject &obj2, Position &pos, wxSize &size) {
		if (&obj == &obj2) {
			return false;
		}
		float SCALE = 2;
		...
		wxRect rect2(static_cast<int>(obj2.getPosition().x * size.x - obj2.getSize().w / SCALE),
					static_cast<int>(obj2.getPosition().y * size.y - obj2.getSize().h / SCALE),
