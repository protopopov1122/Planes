...
MainRefereeController::MainRefereeController(ControllableObject &obj,
                                             GameObjectCollection &game,
                                             float speed,
                                             float initOffsetChangeInterval,
                                             float radius)
    : RefereeController::RefereeController(obj, game, speed),
      chaseOffset(0.0f, 0.0f),
      initOffsetChangeInterval(initOffsetChangeInterval), radius(radius) {}
...
MainRefereeControllerFactory::MainRefereeControllerFactory(
    GameObjectCollection &game, float speed, float initOffsetChangeInterval,
    float radius)
    : RefereeControllerFactory::RefereeControllerFactory(game, speed),
      initOffsetChangeInterval(initOffsetChangeInterval), radius(radius) {}

std::unique_ptr<ObjectController>
MainRefereeControllerFactory::create(ControllableObject &obj) {
  return std::make_unique<MainRefereeController>(
      obj, this->gameObjects, this->speed, this->initOffsetChangeInterval,
      this->radius);
}
}