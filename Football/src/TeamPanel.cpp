...
public:
  ...
  void visit(Player &pl) { this->vec.push_back(&pl); }
...
void TeamPanel::update() {
  ...
  for (Player *pl : vec) {
    ...
    cbox->Bind(wxEVT_CHECKBOX,
               [=](wxCommandEvent &evt) { pl->enable(cbox->GetValue()); });
  }
  this->energySpinner = new wxSpinCtrl(
      this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize,
      wxSP_ARROW_KEYS | wxALIGN_RIGHT, 0, 1000,
      static_cast<int>(this->team.getParameters().getEnergy() * 1000));
