...
namespace Football {
	...
	class KickCorrection : public PlayerVisitor {
		public:
			KickCorrection(Team &team, Position &pos, float angle) : team(team), ppos(pos) {
				this->dx = cos(angle);
				this->dy = sin(angle);
			}
	...
	PlayerController::PlayerController(ControllableObject &obj, TeamCollection &game, Team &team, float speed, float kickStrength)
		: BehaviorController::BehaviorController(obj, game, speed), teams(game), team(team), kickHandler(nullptr), kickStrength(kickStrength) {}
		
	Team &PlayerController::getTeam() {
		return this->team;
	}
	...
	void PlayerController::kick(PassiveObject &object, Position &destination, float speed) {
		...
		float angle = this->kickCorrection(atan2(dy, dx)) + RANDOM_RANGE * 2 / (rand() % 4 + 1) - RANDOM_RANGE;
	...
	void PlayerController::smartKick(PassiveObject &object, Position &position, float speed) {
	...
	PlayerControllerFactory::PlayerControllerFactory(TeamCollection &gameObjects, Team &team, float speed, float kickStrength)
		: gameObjects(gameObjects), team(team), speed(speed), kickStrength(kickStrength) {}
		
	Player::Player(Drawable &draw, Team &team, KickRegister &kickRegister, PlayerControllerFactory &fact, size_t id)
		: GameObject::GameObject(draw), team(team), kickRegister(kickRegister), enabled(true), id(id) {
	...
	Team &Player::getTeam() {
		return this->getBehavior().getTeam();
	}
